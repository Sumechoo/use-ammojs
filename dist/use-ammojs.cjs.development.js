...
var threeAmmo = require('three-ammo');
var threeammoWorker = require('three-ammo/dist/threeammo-worker');
var three = require('three');
var fiber = require('@react-three/fiber');
var ammoDebugDrawer = require('ammo-debug-drawer');

var AmmoPhysicsContext = /*#__PURE__*/React.createContext(null);
function useAmmoPhysicsContext() {
  var context = React.useContext(AmmoPhysicsContext);
...
function ammoDebugOptionsToNumber(debugOptions) {
  var options = ammoDebugDrawer.AmmoDebugConstants.NoDebug;

  if (debugOptions.DrawWireframe) {
    options |= ammoDebugDrawer.AmmoDebugConstants.DrawWireframe;
  }

  if (debugOptions.DrawAabb) {
    options |= ammoDebugDrawer.AmmoDebugConstants.DrawAabb;
  }

  if (debugOptions.DrawFeaturesText) {
    options |= ammoDebugDrawer.AmmoDebugConstants.DrawFeaturesText;
  }

  if (debugOptions.NoHelpText) {
    options |= ammoDebugDrawer.AmmoDebugConstants.NoHelpText;
  }

  if (debugOptions.DrawText) {
    options |= ammoDebugDrawer.AmmoDebugConstants.DrawText;
  }

  if (debugOptions.ProfileTimings) {
    options |= ammoDebugDrawer.AmmoDebugConstants.ProfileTimings;
  }

  if (debugOptions.EnableSatComparison) {
    options |= ammoDebugDrawer.AmmoDebugConstants.EnableSatComparison;
  }

  if (debugOptions.DisableBulletLCP) {
    options |= ammoDebugDrawer.AmmoDebugConstants.DisableBulletLCP;
  }

  if (debugOptions.EnableCCD) {
    options |= ammoDebugDrawer.AmmoDebugConstants.EnableCCD;
  }

  if (debugOptions.DrawConstraints) {
    options |= ammoDebugDrawer.AmmoDebugConstants.DrawConstraints;
  }

  if (debugOptions.DrawConstraintLimits) {
    options |= ammoDebugDrawer.AmmoDebugConstants.DrawConstraintLimits;
  }

  if (debugOptions.FastWireframe) {
    options |= ammoDebugDrawer.AmmoDebugConstants.FastWireframe;
  }

  if (debugOptions.DrawNormals) {
    options |= ammoDebugDrawer.AmmoDebugConstants.DrawNormals;
  }

  if (debugOptions.MAX_DEBUG_DRAW_MODE) {
    options |= ammoDebugDrawer.AmmoDebugConstants.MAX_DEBUG_DRAW_MODE;
  }
...
function Physics(_ref) {
  ...
  React.useEffect(function () {
    ...
    var uuidToIndex = {};
    var bodyOptions = {};
    var ammoWorker = threeammoWorker.createAmmoWorker();
    var workerHelpers = threeAmmo.WorkerHelpers(ammoWorker);
    var sharedArrayBuffer = new SharedArrayBuffer(4 * threeAmmo.CONSTANTS.BUFFER_CONFIG.HEADER_LENGTH + //header
    4 * threeAmmo.CONSTANTS.BUFFER_CONFIG.BODY_DATA_SIZE * threeAmmo.CONSTANTS.BUFFER_CONFIG.MAX_BODIES + //matrices
    4 * threeAmmo.CONSTANTS.BUFFER_CONFIG.MAX_BODIES //velocities
    );
    var headerIntArray = new Int32Array(sharedArrayBuffer, 0, threeAmmo.CONSTANTS.BUFFER_CONFIG.HEADER_LENGTH);
    var objectMatricesIntArray = new Int32Array(sharedArrayBuffer, threeAmmo.CONSTANTS.BUFFER_CONFIG.HEADER_LENGTH * 4, threeAmmo.CONSTANTS.BUFFER_CONFIG.BODY_DATA_SIZE * threeAmmo.CONSTANTS.BUFFER_CONFIG.MAX_BODIES);
    var objectMatricesFloatArray = new Float32Array(sharedArrayBuffer, threeAmmo.CONSTANTS.BUFFER_CONFIG.HEADER_LENGTH * 4, threeAmmo.CONSTANTS.BUFFER_CONFIG.BODY_DATA_SIZE * threeAmmo.CONSTANTS.BUFFER_CONFIG.MAX_BODIES);
    objectMatricesIntArray[0] = threeAmmo.CONSTANTS.BUFFER_STATE.UNINITIALIZED;
    var debugSharedArrayBuffer = new SharedArrayBuffer(4 + 2 * ammoDebugDrawer.DefaultBufferSize * 4);
    var debugIndex = new Uint32Array(debugSharedArrayBuffer, 0, 4);
    var debugVertices = new Float32Array(debugSharedArrayBuffer, 4, ammoDebugDrawer.DefaultBufferSize);
    var debugColors = new Float32Array(debugSharedArrayBuffer, 4 + ammoDebugDrawer.DefaultBufferSize, ammoDebugDrawer.DefaultBufferSize);
    var debugGeometry = new three.BufferGeometry();
    ...
    ammoWorker.postMessage({
      type: threeAmmo.CONSTANTS.MESSAGE_TYPES.INIT,
      worldConfig: removeUndefinedKeys({
        debugDrawMode: ammoDebugOptionsToNumber(drawDebugMode),
        gravity: gravity && new three.Vector3(gravity[0], gravity[1], gravity[2]),
        epsilon: epsilon,
          ...
          return runtime_1.wrap(function _callee$(_context) {
            while (1) {
              switch (_context.prev = _context.next) {
                case 0:
                  if (event.data.type === threeAmmo.CONSTANTS.MESSAGE_TYPES.READY) {
                    resolve({
                  ...
                  } else if (event.data.type === threeAmmo.CONSTANTS.MESSAGE_TYPES.BODY_READY) {
  ...
  fiber.useFrame(function () {
    ...
    if (Atomics.load(headerIntArray, 0) === threeAmmo.CONSTANTS.BUFFER_STATE.READY) {
      for (var i = 0; i < uuids.length; i++) {
        var uuid = uuids[i];
        var type = bodyOptions[uuid].type ? bodyOptions[uuid].type : threeAmmo.CONSTANTS.TYPE.DYNAMIC;
        var object3D = object3Ds[uuid];

        if (type === threeAmmo.CONSTANTS.TYPE.DYNAMIC) {
          matrix.fromArray(objectMatricesFloatArray, uuidToIndex[uuid] * threeAmmo.CONSTANTS.BUFFER_CONFIG.BODY_DATA_SIZE);
          ...
          objectMatricesFloatArray.set(object3D.matrixWorld.elements, uuidToIndex[uuid] * threeAmmo.CONSTANTS.BUFFER_CONFIG.BODY_DATA_SIZE);
        } // print velocities
        // console.log(
        //   uuid,
      ...
      Atomics.store(headerIntArray, 0, threeAmmo.CONSTANTS.BUFFER_STATE.CONSUMED);
    }
    /* DEBUG RENDERING */
  ...
  return React__default.createElement(AmmoPhysicsContext.Provider, {
  ...
  }, drawDebug && React__default.createElement("lineSegments", {
    geometry: debugGeometry,
    frustumCulled: false,
    renderOrder: 999
  }, React__default.createElement("lineBasicMaterial", {
    attach: "material",
    vertexColors: true,
    depthTest: true
  })), children);
...
Object.defineProperty(exports, 'BodyType', {
  enumerable: true,
  get: function () {
    return threeAmmo.BodyType;
  }
});
Object.defineProperty(exports, 'ConstraintType', {
  enumerable: true,
  get: function () {
    return threeAmmo.ConstraintType;
  }
});
Object.defineProperty(exports, 'ShapeFit', {
  enumerable: true,
  get: function () {
    return threeAmmo.ShapeFit;
  }
});
Object.defineProperty(exports, 'ShapeType', {
  enumerable: true,
  get: function () {
    return threeAmmo.ShapeType;
  }
});
Object.defineProperty(exports, 'AmmoDebugConstants', {
  enumerable: true,
  get: function () {
    return ammoDebugDrawer.AmmoDebugConstants;
  }
});
exports.AmmoPhysicsContext = AmmoPhysicsContext;
exports.Physics = Physics;
exports.useAmmoPhysicsContext = useAmmoPhysicsContext;
exports.usePhysics = usePhysics;
//# sourceMappingURL=use-ammojs.cjs.development.js.map
